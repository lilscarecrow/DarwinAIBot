...
**`/tournament` (2026-09-04):** toggles `tournament_mode` (config, default `false`) — persisted via `DirectorCog._persist_config_value()`, a generalized version of `ScrimCog._save_message_id()`'s read-modify-write pattern, so it survives a bot restart. Valid in any state (only `_role_check`, no `_state_check`) since it's a pure config flag, not tied to session/match state. Behavior changes while it's on:
- **`/custom`'s OBS stream start is NOT delayed (2026-09-12, reversed — the original design, then a follow-up fix for it, both removed the same day).** This bot originally waited `_TOURNAMENT_STREAM_DELAY_SECONDS` (120s) before going live under tournament mode, on the theory that a short delay would stop viewers from relaying real-time positions to players; a same-day follow-up (`obs_control.is_streaming()` + stopping an already-live stream so the delay would actually re-apply) tried to close a gap in that. Neither survived: delaying only the *start* of a stream hides the first couple minutes and does nothing for the other 90%+ of a match — it was never the right mechanism for "no near-real-time info leaks." A genuine fix needs a **continuous broadcast delay** covering the whole match: Twitch's own `channel.delay` (settable via the same Modify Channel Information API the stream-title toggle below uses) requires **Partner status**, which this account doesn't have; OBS's local Advanced → Stream Delay setting works on any account but **isn't exposed via `obs-websocket`** at all (no request to read or set it), so the bot can't drive it either. The actual mitigation now is manual and outside the bot: enable OBS's own Stream Delay yourself if you want real protection — `/tournament on`'s response embed says so plainly rather than implying the bot handles it. `/custom` just starts the stream immediately under tournament mode, same as every other case.
...
- **The results screenshot is never uploaded to darwinstalker (2026-09-10)** — tournament matches aren't counted as scrims, so `_post_results_to_ingest()` no-ops (logs and returns `None`, same as its existing "no screenshot" case) before calling `DraftLifecycle.post_results()` whenever `tournament_mode` is on. This only skips the ladder upload; `_mirror_results()` (posting the screenshot to Discord's `results` channel) runs unconditionally at both call sites, exactly as before — a tournament match's results still show up in Discord, just never on the ladder. Callers already treat a `None` return the same way they'd treat a genuine upload failure (`_resolve_prediction()` cancels/refunds the Twitch prediction for lack of `placements` rather than guessing), so nothing downstream needed to change.
- **The lobby-ready embed goes directly back to whoever ran `/custom`, not the public lobby-ping channel (2026-09-10)** — see the matching note under `/custom`'s Commands section above.
- **The scrim signup queue is taken down entirely (2026-09-11)** — a tournament lobby isn't part of the normal open-signup scrim rotation, so there should be nothing left for a viewer to react to while one is running. Turning tournament mode **on** calls `ScrimCog.suspend_signup()`, which deletes both the signup message and the region-breakdown message below it (no repost — unlike `/role remove`'s `_delete_and_repost_signup_message()`, which always brings a fresh one right back), clears `_signup_order` and the reset countdown, and clears `scrim_signup_message_id`/`scrim_region_message_id` back to `null` in config. Turning it **off** calls `ScrimCog.resume_signup()`, which just calls `_ensure_signup_message()`/`_ensure_region_message()` — since the ids are `null`, both take their normal "no tracked message yet" path and post fresh ones, same as a first-ever run. `ScrimCog.on_ready()` checks `tournament_mode` first and skips its own startup message-recreation entirely while it's `true`, so a bot restart mid-tournament doesn't resurrect the queue behind tournament mode's back — `resume_signup()` (i.e. `/tournament off`) is the only path back. `/tournament`'s own handler now defers (`interaction.response.defer(thinking=True)`, followed by `interaction.followup.send()`) since these deletes/posts are the same class of real Discord API work `/role add`/`/role remove` already defer for.
- **The Twitch stream title switches with tournament mode, from two fixed config values — no fetching or stashing (2026-09-12, simplified 2026-09-14)** — turning tournament mode **on** pushes `twitch_tournament_stream_title` (config, default `_DEFAULT_TOURNAMENT_STREAM_TITLE` = `"$1000 Hotdog Hoedown Tournament"`); turning it **off** pushes `twitch_default_stream_title` (config, the normal day-to-day scrim title). Both directions just call `DarwinTwitchBot.set_stream_title(...)` (Modify Channel Information) with whichever value is configured — no-op (no title change attempted) if `self.bot.twitch_bot` is `None` (`twitch_enabled: false`), and a warning note in the response embed if the target config key is empty. Requires the `channel:manage:broadcast` scope (added to `_OAUTH_SCOPES` in `bot/twitch_bot.py` — re-authorize once via `_OAUTH_URL`, same as every other scope addition there). Works whether or not the channel is currently live.

**Replaced a fetch-and-restore design that could restore a stale title (found live, 2026-09-13, removed 2026-09-14):** the original version fetched whatever the title happened to be right before `/tournament on` ran, stashed it (`twitch_pre_tournament_stream_title`, persisted so a bot restart mid-tournament didn't lose it), and restored that exact value on `/tournament off`. The bug: if the stashed title already contained event-specific promotional text (e.g. a "$1,000 TOURNAMENT SIGNUPS: ..." announcement posted *before* the bracket started), `/tournament off` faithfully restored that stale text, signup link included, even once the tournament was long over — not a bug in the stash/restore logic itself, just an inherent risk of restoring "whatever was there before" instead of a known-good value. Rather than patch the staleness detection, the whole mechanism was replaced with the simpler two-fixed-values design above: with nothing to fetch or stash, there's nothing that can go stale — `/tournament off` always pushes the same deliberately-chosen `twitch_default_stream_title`, every time. `DarwinTwitchBot.fetch_stream_title()` was removed entirely along with it (no other caller). This also means `twitch_default_stream_title` is no longer just a reference value someone updates by hand on the Twitch dashboard — it's live, read every `/tournament off`, so keep it current.

**The title was only ever re-pushed reactively, on an explicit `/tournament` toggle — a stream could go live still showing a stale title from a previous session (found live, 2026-09-14, same day as the fix above):** the config-driven design fixed *what* gets pushed, but the push itself only ever happened inside `/tournament`'s command handler. A stream started via `/custom` without anyone having toggled `/tournament` since the fix landed just kept whatever title was already live — in this case, promotional tournament-era text from 09-12, untouched by the new mechanism simply because nothing had triggered it yet. `DirectorCog._push_tournament_stream_title(tournament_mode)` (extracted from `/tournament`'s handler into a shared method) is now also called from `/custom`'s stream-start success path, right alongside `_set_streaming_presence()` — so the title self-corrects to match the current `tournament_mode` the moment any stream actually goes live, not only when `/tournament` happens to be toggled. `/tournament`'s handler is unchanged in behavior, just now calling the shared method instead of the same logic inline.
- **The OBS "Game N" banner is hidden while it's on (2026-09-12)** — `DraftLifecycle.game_index` counts games within a real ladder set (seeded from the server's `next_game_index`, see the Ladder Ingestion section), and a tournament lobby isn't that, so the number it would show doesn't track reality here. Rather than let a stale/meaningless count sit on screen, `/tournament`'s handler calls `obs_control.set_source_visible(obs_game_number_source, not value)` directly — the same call the minimap cover uses, wrapped in `run_in_executor` since it's a blocking websocket call — hiding the source on **on** and showing it again on **off**. No-ops if `obs_stream_enabled` is `false`. `_update_game_number_banner()`'s own text-setting logic no longer runs at all under `tournament_mode` either — see the next bullet.
- **No ladder draft/set is opened for tournament matches at all (2026-09-12, reversed from the 2026-09-04 tagged-draft design), and the "Who wins?" Twitch prediction is disabled too.** Previously `/tournament on slug:<slug>` tagged every draft opened while tournament mode was on with a darwinstalker.com tournament slug; that's gone — a tournament match now never touches the ladder's draft system, tagged or not, and the `slug` parameter was removed from `/tournament` entirely (the persisted `ds_ingest_tournament_slug` config key is simply unused now). Two call sites had to change:
  - `DirectorCog._open_ds_draft()` (`/custom`'s success path) returns immediately, before calling `DraftLifecycle.open_lobby()` at all, whenever `tournament_mode` is `true`.
  - `MatchRunner._init_player_bar_and_push()` skips its entire `self._ds` block (`on_match_start()`, the `match_start` event, and `_update_game_number_banner()`) under `self._tournament_mode` — not just the first of these. This one matters beyond symmetry: `DraftLifecycle.on_match_start()` self-opens a fresh (untagged) draft the instant it sees no draft id yet (see game/ds_lifecycle.py's own docstring — this is a safety net for the normal auto-start path), so calling it unconditionally here would silently defeat `_open_ds_draft()`'s skip the moment a tournament match actually started. Covered by `tests/test_match_runner_player_bar_init.py`'s tournament_mode tests.
  - `DirectorCog._maybe_open_prediction()` also returns immediately when `tournament_mode` is `true`. Reason: with no draft ever opened, `self._ds.known_players`/`unlinked`/`roster_size` all stay empty (they're populated from the draft-open/roster-push response — see `DraftLifecycle`'s `_lobby` field), so `MatchRunner.is_lobby_captured()`'s name resolution has nothing server-confirmed to check names against. Rather than open a prediction on a lobby capture that can't actually be vouched for, tournament matches just don't get one.
  - `_close_ds_draft()` and `_post_results_to_ingest()` needed no changes — the former already no-ops cleanly when `_draft_id` is `None` (nothing was ever opened to close), and the latter already skipped the upload under `tournament_mode` (see above) regardless of whether a draft existed.
- **The end-of-match Twitch ad break is skipped too (2026-09-12)** — see the Ad break automation note under the Twitch Chat Bot section below for why (an OBS broadcast delay, set manually since the bot can't set one itself, means a wall-clock-timed ad would land on whatever moment the *delayed* stream is showing, most likely still mid-match for viewers).

**`/custom`'s full details embed now goes to the lobby-ping channel, not the command sender (2026-09-07) — except under `tournament_mode` (2026-09-10, reverted back for that case):** the "Custom Match Ready" embed (region, profile, lobby code, ladder-claim nudge, Twitch stream status) is sent to `_LOBBY_PING_CHANNEL_ID` as `ping_ch.send(content=f"<@&{_LOBBY_PING_ROLE_ID}>", embed=embed)` — same role ping as before, now carrying the embed instead of a bare `<role> <code>` text line. The command sender instead gets a short `/launch`-style confirmation (`self._ok("Custom Match Ready", "Private lobby created. Run \`/start\` to begin the match.")`) telling them what to do next, mirroring `/launch`'s "Run `/custom` to create a match." response. Everything else in `/custom`'s success path (OBS stream start, ladder draft open, Twitch announce, minimap cover, auto-start watcher) is unchanged — only which channel gets which message moved.

Under `tournament_mode`, this is reverted back to the original (pre-2026-09-07) behavior: the full details embed — lobby code included — is sent directly as the `/custom` response to whoever ran the command, and the lobby-ping channel is skipped entirely (no role ping either). Tournament matches aren't a public open-signup queue the way regular scrims are — the organizer running `/custom` is expected to relay the code to participants themselves, so there's no "whole server should see this" audience to page.

**Auto-start watcher (`DirectorCog._watch_for_auto_start()`):** started right after `/custom` creates a lobby — OCRs the "CUSTOM MATCH EXPIRES IN MM:SS" banner, sleeps until it expires, then fires the match runner automatically (`skip_start=True`, no B-press needed since the game already started the countdown itself). Cancelled immediately if `/start` is called manually first. `/status` shows time remaining via `self._lobby_expiry`, set the moment the countdown is confirmed and cleared once the wait begins (`None` again while actually sleeping, so `/status` doesn't show a stale "expiry" during that window — see the field's own two assignments).

**A single-digit minutes OCR read is reconstructed, not trusted as-is (2026-09-10 fix, found live):** one read of the "CUSTOM MATCH EXPIRES IN MM:SS" banner came back `"(9:47"` instead of the true `"19:47"` — the leading `1` misread as a stray `(` character — and the watcher, trusting it, fired a match roughly 10 minutes early. This wasn't a regression: `git log` shows `read_lobby_countdown()` (`game/ocr.py`) hadn't been touched since it was first written, and grepping months of `logs/darwin_bot.log` found this exact `"9:XX"`-instead-of-`"19:XX"` misread already recurring intermittently, always alongside a correct `"19:XX"` read moments before/after — it simply hadn't previously landed on a live watcher invocation. The log also showed the tell that makes the fix possible: across hundreds of legitimate reads, minutes is *always* `19` — never `18`, never `20` — because this is only ever read moments after `/custom` creates the lobby, against its own fixed ~20-minute timeout. So `read_lobby_countdown()` now reconstructs a captured single-digit minutes value as `10 + that digit` rather than trusting it (a dropped leading `1` is the only way a single digit ever shows up here) — fixing the misread at its source instead of trying to detect it after the fact. `DirectorCog._read_lobby_countdown()` (renamed from `_read_confirmed_lobby_countdown()`, a first version of this fix that took a second OCR read `_COUNTDOWN_CONFIRM_GAP_SECONDS` later and compared it against the first) is back down to a single read, with `_COUNTDOWN_MIN_PLAUSIBLE_SECONDS` (900s / 15 min) kept as a final sanity floor in case OCR ever fails some other, not-yet-seen way. See `tests/test_ocr_countdown.py` (mocks `pytesseract.image_to_string` directly with the real log strings, including the exact `"(9:47"` misread) for the reconstruction's test coverage.
...
**POV locked from lobby creation through match startup (2026-09-09, widened same day after a live miss):** `/pov`, `!pov`, and the channel-points redemption all additionally check `session.is_pov_locked()` (on top of the existing `is_command_valid("pov")` state check) and refuse — ephemeral reply on Discord, a chat reply on Twitch, a silent refund on the redemption, same as any other rejected attempt — while it's `True`. Why: a manual POV switch enlarges a different card's name band (the "spectated" card is drawn bigger, per `game/player_cards_v2.py`), which would corrupt `_log_slot_map_snapshot()`'s OCR read taken at match start.

**First version locked too late:** `SessionState._pov_locked` was originally set only at `MatchRunner` construction (`/start`, the auto-start watcher). Found live the same day — a viewer's `!pov` during the *lobby* (before `/start`) went through unblocked, and that switch persisted into match start and could still corrupt the snapshot, since the same player-bar UI (and its spectate-camera mechanic) is already live in the lobby, not just mid-match. **Current:** `DirectorCog.custom()`'s success branch locks it the moment the lobby is created — `MatchRunner.__init__` still locks it too (belt and suspenders, redundant but harmless for the normal `/start` path). Unlocked on whichever of three paths actually ends the lobby: `run()`'s forced default-POV-1 press (the match started normally), `_reset_session()`'s universal reset-to-`IDLE` via `SessionState.reset()` (the match/lobby was aborted), or `/menu`'s success branch (the lobby was abandoned without ever starting a match — the one `IN_CUSTOM` exit that goes through neither of the other two, so it unlocks explicitly). Between those three, POV can never end up stuck refusing for the rest of the session.
...
| `twitch_pov_reward_title` | `"Change POV"` — the reward's title; the bot creates it via API under this title if a manageable one doesn't already exist, see Channel points POV reward below |
| `twitch_pov_reward_cost` | `250` — channel-points cost, only used the first time the bot creates the reward (editable afterward in the Creator Dashboard; the bot never re-syncs cost on restart) |
| `twitch_favorite_reward_title` | `"Crowd Favorite"` — same API-created pattern as the POV reward, see Crowd Favorite reward below |
| `twitch_favorite_reward_cost` | `500` — channel-points cost, same "only read the first time it's created" caveat as `twitch_pov_reward_cost` |
| `twitch_predictions_enabled` | `false` by default — see Twitch "Who Wins?" Prediction below |
| `twitch_tournament_stream_title` | Stream title pushed by `/tournament on` — default `"$1000 Hotdog Hoedown Tournament"` (`_DEFAULT_TOURNAMENT_STREAM_TITLE` in `discord_bot.py`), see `/tournament`'s Commands section above |
| `twitch_default_stream_title` | Stream title pushed by `/tournament off` (2026-09-14, previously reference-only) — the normal, non-tournament scrim title. Edit this whenever the day-to-day title should change; `/tournament off` reads it live, every time |
...
**Skipped entirely under `tournament_mode` (2026-09-12):** with a tournament's OBS broadcast running several minutes behind real time (a manually-configured OBS Stream Delay — see the `/tournament` note above for why the bot can't set this itself), firing the ad by wall-clock time at real-match-end lands the ad on whatever moment the *delayed* broadcast happens to be showing right then — almost certainly still mid-match action for viewers, cutting into the finish rather than covering genuine downtime. There's no way to compensate by shifting the ad's own timing either: Twitch's Start Commercial API takes no schedule, only "start now." So `_twitch_start_ad_break()` just no-ops under `tournament_mode`, same as the other tournament-mode checks in this cog.
...
**`event_custom_redemption_add()` dispatches by reward title (2026-09-10, generalized when Crowd Favorite was added):** handles `channel.channel_points_custom_reward_redemption.add`, subscribed with no `reward_id` filter (so it fires for *every* custom reward redeemed on the channel), and routes `payload` to `_handle_pov_redemption()` or `_handle_favorite_redemption()` by matching `payload.reward.title` against `twitch_pov_reward_title`/`twitch_favorite_reward_title` (case/whitespace-insensitive) — any other reward on the channel is ignored entirely. A shared `_refund(payload, reason)` helper (logs why, then `payload.refund(token_for=...)`, swallowing any failure) is used by both handlers.

**Channel points "Change POV" reward (2026-09-04):** `_handle_pov_redemption()` requires **"Require viewer to enter text" enabled** on the reward, so `payload.user_input` carries the player number — same `1`-`9`/`0` choices as `/pov` and `!pov`. Redemptions default to `UNFULFILLED` and sit in the dashboard queue forever unless explicitly updated, so this always resolves one of two ways: `payload.fulfill(token_for=...)` on a successful `press_key()` call, or a refund (points returned to the viewer) if the input was invalid, the game state doesn't allow POV changes right now, or the keystroke send failed — fulfill requires `channel:manage:redemptions`.

**The reward must be created via this app's own API call, not the Creator Dashboard (2026-09-10 fix, found live):** originally documented as "create it manually in the Creator Dashboard" — that turned out to make `fulfill()`/`refund()` **fail 100% of the time**, from the day the feature shipped, with `403: The ID in the Client-Id header must match the client ID used to create the custom reward, or the broadcaster doesn't have partner or affiliate status`. Twitch's Update Redemption Status API only allows an app to fulfill/refund a reward's redemptions if that same app's Client-ID created the reward — a Dashboard-created reward belongs to Twitch's own first-party web client and can never be managed by any third-party app, regardless of scopes granted (Affiliate/Partner status wasn't the issue here — ad breaks, which genuinely require it, were already working). The silent failure of `refund()` was the worse half of this: Twitch deducts a viewer's points at redemption time regardless of what happens next, so every redemption this code tried to refund (invalid input, cooldown, a failed keystroke send) never actually returned those points — the bot logged a refund attempt, but the viewer kept losing them.

**Fix:** `DarwinTwitchBot._ensure_pov_reward()` creates the reward itself via `PartialUser.create_custom_reward()` (Twitch's Create Custom Reward API) instead of expecting it to already exist — called from both `setup_hook()` and `event_oauth_authorized()`, the same retry points as `_subscribe_chat()`/`_subscribe_events()`. Idempotent: checks `fetch_custom_rewards(manageable=True)` first (rewards this app's Client-ID can already manage) and does nothing if one with the configured title is already there, so restarting or re-authorizing never creates a duplicate. `twitch_pov_reward_cost` (config, default `250`) is only read the first time the reward is created — editing the cost afterward is a normal Creator Dashboard edit, the bot doesn't try to keep it in sync. **One-time step if migrating an existing Dashboard-created reward:** delete it in the Creator Dashboard first — Twitch requires reward titles to be unique per channel, so creating the API-owned one alongside the old one fails with a duplicate-title error; `_ensure_pov_reward()` logs that plainly if it happens.

**Redemption text is parsed, not exact-matched (2026-09-07):** `_handle_pov_redemption()` (and, since 2026-09-10, `_handle_favorite_redemption()` too — both need the same "which player" input) runs `payload.user_input` through `_extract_pov_key()` instead of checking it against `_VALID_POV_KEYS` directly. Found live — viewers used to typing chat commands kept typing `!pov 5` or `pov 5` into the redemption's text box instead of the bare `5` the reward asks for, and those got refunded as "invalid" even though the intent was clear. `_extract_pov_key()` strips an optional leading `!`/`pov` prefix (`_POV_PREFIX_RE = re.compile(r"^!?\s*pov\s*", re.IGNORECASE)`, case-insensitive, with or without a space before the digit) before checking the remainder against `_VALID_POV_KEYS`, so `"5"`, `"!pov 5"`, `"pov 5"`, and `"POV5"` all resolve to `"5"`; anything that still doesn't land on a valid key (typos, extra words, empty input) returns `None` and is refunded exactly as before — no channel points are consumed for a redemption that doesn't resolve. Scoped to the channel-points path only: `!pov` in chat is already split into command + argument by twitchio itself and never touches this parser.

**Global cooldown for viewer redemptions, mods/broadcaster bypass (2026-09-04, lowered to 30s 2026-09-07):** `_handle_pov_redemption()` rejects (refunds) a redemption from a non-privileged viewer if it comes within `_POV_REDEMPTION_COOLDOWN_SECONDS` (30s) of the last one that actually changed POV — a plain module-level cooldown, not per-viewer, so it caps how often POV can flip regardless of who's spamming the reward. This cooldown is POV-specific — Crowd Favorite (below) has none; its own deck-scarcity check is the rate limiter there. `_is_mod_or_broadcaster(user_id)` decides privilege: the broadcaster always passes (`user_id == owner_id`), otherwise it calls `PartialUser.fetch_moderators(user_ids=[user_id])` (requires the new `moderation:read` scope, added to `_OAUTH_SCOPES` — needs the same one-time re-authorization as every other scope addition here) and treats any lookup failure as **not privileged**, so the cooldown fails safe (stays enforced) rather than silently opening up if the scope isn't granted yet. Privileged users skip the cooldown check entirely — this is deliberately enforced in bot code rather than via the reward's own Twitch-side cooldown setting, since Twitch's built-in per-reward cooldown applies to everyone with no concept of a mod bypass. The cooldown only arms after a redemption actually sends a successful keystroke (`sent and not is_privileged`) — a failed send or invalid input doesn't burn the window. None of this touches `/pov` or `!pov`: both remain mod/admin-gated with **no** cooldown, exactly as before — this cooldown exists solely on the channel-points path, which is the one open to arbitrary viewers.

**Channel points "Crowd Favorite" reward (2026-09-10):** `_handle_favorite_redemption()` — same player-number input and `_extract_pov_key()` parsing as the POV reward, but a completely different action: instead of a raw keystroke, it queues a **favorite_player card play** on the live match, via `game/match_runner.py`'s `MatchRunner.try_queue_favorite_reward(player_index)` / `_maybe_fire_favorite_reward()`. Reaching the runner is why `DarwinTwitchBot.active_runner` exists: `DirectorCog._set_active_runner()` (`bot/discord_bot.py`) sets it alongside the cog's own `_active_runner` at every match start/end call site, so the Twitch bot always has (or lacks) a reference matching whether a match is actually in progress. No match in progress (`active_runner is None`), an unparseable redemption, or `try_queue_favorite_reward()` returning `False` all refund; otherwise the redemption is **fulfilled immediately once queued** — the queue/fire delay (waiting for a safe moment to actually play the card, anywhere from instant to several minutes) is a bot-internal detail, not something that should leave a valid redemption sitting `UNFULFILLED` in the dashboard.

**`try_queue_favorite_reward(player_index)`** (on `MatchRunner`, called cross-thread from the Twitch bot's event loop — plain attribute mutation, no lock, same convention as this class's other cross-thread flags) accepts only if **all** of: nothing is already queued (only one redemption is ever allowed pending at a time, by explicit design choice — a second copy existing in the deck doesn't matter if one is already queued, a viewer must wait for the first to resolve), `player_index` is a real, currently-alive card index in the match-start roster, and at least one `favorite_player` card remains unplayed in the deck. No cooldown, unlike POV — deck scarcity (typically 2 copies in the starting deck) is the natural rate limiter here.

**Does NOT require a captured name (2026-09-10 fix, found in review):** unlike first blood, this flow never resolves a name at all — the viewer names the *slot* directly via `/pov`-style digit input, parsed the same way `/pov`'s own redemption is. The acceptance check originally also required `self._player_names[player_index]` to be non-empty, rejecting a real, currently-alive player whose nameplate simply failed to OCR at match start — a stricter requirement than the flow actually needs, since nothing downstream depends on the name being present (`_slot_name()` already falls back to `"slot N"` for logging/TTS). Now checks only the index range and `self._player_alive[player_index]`.

**`_maybe_fire_favorite_reward(elapsed, card_schedule)`**, called every main-loop iteration right after the first-blood reward's own check, mirrors `_maybe_fire_first_blood_reward()`'s three-part gate almost exactly (target still alive, no real scheduled card due within `_BONUS_CARD_TIMING_BUFFER_SECONDS` — a constant now shared by both reward flows) but **skips the points check entirely**: `favorite_player` costs 0 director points, so there's nothing to wait on. The one behavioral difference from first blood: resolving back to `None` (not a permanent "resolved" latch) once fired, canceled, or found unplayable — Crowd Favorite is a repeatable reward, so a brand new redemption must be immediately accept-able again the instant this one settles, unlike first blood (a genuine one-time match event).

**Disallowed once the card schedule is exhausted (2026-09-10, global — not tournament-specific):** the schedule always ends with a zone_close, which has no verification at all (see Zone Logic below) — if the match is already down to one zone by the time it fires, the game silently rejects it, harmless only because nothing else is scheduled afterward to be thrown off by it. Crowd Favorite isn't tied to the schedule the way zone_close is, so without a check it could still be redeemed well after the last card — right when the match is likely down to its final zone/players and a POV-switch-then-drag is at its least predictable. `MatchRunner._past_last_scheduled_card()` (`bool(self._card_schedule) and all(e.done for e in self._card_schedule)` — `run()` stashes the built schedule as `self._card_schedule` specifically so this and `try_queue_favorite_reward()` can both see it; an empty/unset schedule, e.g. before `run()` builds one, is treated as "not exhausted," not "exhausted") gates both `try_queue_favorite_reward()` (new redemptions refused, same as any other rejection) and `_maybe_fire_favorite_reward()` (an already-queued reward is canceled outright, same handling as the target dying — covers being queued just before the last card fired and still pending once it has).

Both this and first blood's give_wood share the actual "give the card" mechanics — `_give_reward_card(card_type, player_index, deck_pos, tts_prefix, event_label)`, extracted 2026-09-10 when Crowd Favorite was added (see the give_wood section above for the POV-lock/center-drop reasoning, identical for both) — `_give_first_blood_reward()` and `_give_favorite_reward()` are now both thin one-line wrappers around it.

**`advanced_cards` (config, default `true`) is the master toggle for both bonus-card rewards (2026-09-10):** checked in `MatchRunner._advanced_cards_enabled`, set once at construction. `_maybe_queue_first_blood_reward()` no-ops and `try_queue_favorite_reward()` returns `False` (the Twitch bot refunds, same as any other rejection) when it's off — the two natural "queue" entry points, since nothing downstream (`_maybe_fire_first_blood_reward`/`_maybe_fire_favorite_reward`) ever has anything to act on if nothing was ever queued. `DarwinTwitchBot._ensure_favorite_reward()` also skips creating the Crowd Favorite reward on Twitch entirely when it's off, so viewers never see a redeemable reward that could only ever refund; `_ensure_pov_reward()` (a separate, pre-existing feature) is untouched by this toggle. Deliberately does **not** gate `_poll_damage_feed()`'s `feed_first_blood`/`feed_kill` event logging to the ladder — that's informational data with no gameplay side effect, not a card, so it keeps running regardless of this setting.

**Two fixes from reviewing both reward flows together (2026-09-10):**
1. **`_maybe_fire_favorite_reward`'s timing check could read a stale `elapsed`.** Both reward checks in the main loop used the single `elapsed` value captured at the top of that loop iteration — but if first blood's reward fired first in the *same* iteration, it blocks for its own ~1-2s drag before Crowd Favorite's check runs, so that check was judging "is a scheduled card due soon" against reality as it was before the drag, not as it is now (understating how close a real card actually is by however long the first drag took). Fixed by recomputing `elapsed = time.monotonic() - start_time` between the two calls in `run()`'s main loop.
2. **No delay between the POV switch and the card drop.** `_give_reward_card()` pressed the POV key and immediately proceeded to the drag, with `verify_card_plays: false` (this repo's own setting) leaving zero deliberate delay in between — resting entirely on an unverified assumption that Darwin Project applies a spectate-target switch instantly, when a "give X to player" card's targeting depends on that switch having actually landed first. `_POV_SWITCH_SETTLE_SECONDS` (0.3s, a defensive margin, not a measured value) is now waited out after the press and before the drag — via `self._stop.wait()`, so a force-stop landing during that brief window aborts the play (skips the drag) instead of dragging into a match that's being torn down, same as every other wait in this class.

**Left as-is after review:** Crowd Favorite fulfills the Twitch redemption the moment it's *queued* (`try_queue_favorite_reward()` returns `True`), not once the card actually fires — if the target dies (or the deck's last copy is otherwise claimed) during the wait for a safe moment to fire, the reward is silently canceled but the viewer's redemption stays `FULFILLED`, points already spent, nothing delivered. Fixing this properly would mean holding `fulfill()`/`refund()` until `_maybe_fire_favorite_reward()` actually settles the reward, which needs the `payload` (or its fulfill/refund closure) threaded from the Twitch bot's event loop into `MatchRunner`, and a way to call back into that event loop from the match thread (`asyncio.run_coroutine_threadsafe`) — a real architecture change, deliberately not made; accepted as a known trade-off rather than fixed.
...
5. **All scopes this bot ever requests are combined into one constant**, `_OAUTH_SCOPES` in `bot/twitch_bot.py` (`_OAUTH_URL` is the full authorize URL built from it) — currently `user:bot`, `user:read:chat`, `user:write:chat`, `channel:bot` (chat), `channel:edit:commercial` (ads), `channel:read:subscriptions` (sub/resub/gift-sub shoutouts), `bits:read` (cheer shoutouts), `channel:read:redemptions` + `channel:manage:redemptions` (the channel-points POV reward), `moderation:read` (checking whether a channel-points redeemer is a mod, for the cooldown-bypass logic above), `channel:manage:predictions` (the "who wins" match prediction, see below), `channel:manage:broadcast` (reading/setting the stream title for tournament_mode's title toggle, see `/tournament`'s Commands section above). Visiting `_OAUTH_URL` (`http://localhost:4343/oauth?scopes=...`, logged automatically whenever any subscription attempt fails from a missing scope) grants all of them in one authorization — if the bot account and broadcaster are the same Twitch account (the current setup), one grant covers everything. Adding a new scope-gated feature later: add the scope to `_OAUTH_SCOPES` and re-authorize once; every existing grant stays valid, nothing needs to be redone.
6. Neither the "Change POV" nor "Crowd Favorite" channel-points reward needs manual setup — `_ensure_pov_reward()`/`_ensure_favorite_reward()` create them via the API on startup if they don't already exist (see Channel points POV reward / Crowd Favorite reward below for why they must be API-created, not made in the Creator Dashboard).
...
### Twitch "Who Wins?" Prediction (2026-09-09)

One Twitch Prediction per match — one outcome per captured player, `"{slot} - {name}"` — opened once the match-start slot map is fully captured, resolved against the ladder's OCR'd placement once the match ends. Config-gated: `twitch_predictions_enabled` (default `false`, currently `true` in this repo's `config.json`), no-ops entirely if `false` or `bot.twitch_bot` is `None`.

**Twitch side** (`DarwinTwitchBot` in `bot/twitch_bot.py`, wrapping TwitchIO 3.x's already-built `PartialUser.create_prediction()`/`end_prediction()` — no raw HTTP needed):
- `create_prediction(title, outcomes, prediction_window)` → the `Prediction` object (`.id`, `.outcomes[].id/.title`) or `None` on failure. Twitch allows only one active prediction per channel at a time — a failure here (including "one's already open") surfaces as a caught exception, same as any other rejection.
- `end_prediction(prediction_id, status, winning_outcome_id=None)` — `status` is `"RESOLVED"` (needs `winning_outcome_id`), `"CANCELED"` (full refund), or `"LOCKED"`.
- Requires the `channel:manage:predictions` scope (added to `_OAUTH_SCOPES` — re-authorize once via `_OAUTH_URL`, same as every other scope addition here).

**Opening** (`DirectorCog._maybe_open_prediction(runner)`, fired via `asyncio.ensure_future` right after both `/start` and the auto-start watcher set `self._active_runner = runner`, so it never blocks match start): waits on `runner._slot_map_ready` (bounded by `_PREDICTION_WINDOW_SECONDS`, the same constant as the betting window — no reason to wait longer for the snapshot than the prediction would stay open anyway), then requires `runner.is_lobby_captured()` — a partial capture means **no prediction this match**, not a guess. Outcome titles are `"{slot} - {name[:21]}"` (Twitch caps outcome titles at 25 chars; slot is always 1 digit so 21 leaves room for the `"N - "` prefix), capped at the first 10 slots (Twitch's own outcome-count ceiling) and skipped entirely below 2 (Twitch's floor — shouldn't happen given the game's own player minimum, but guarded). The resulting `Prediction`, the slot map used to build it, and a `{slot: outcome_id}` lookup (matched by exact outcome *title* string against `{outcome.title: outcome.id for outcome in prediction.outcomes}` rather than by position/order, since Twitch's returned outcome order isn't a documented guarantee) are stashed on the cog as `_active_prediction`/`_prediction_slot_map`/`_prediction_outcome_by_slot`.

**Resolving** (`DirectorCog._resolve_prediction(placements)`, called right after `_post_results_to_ingest()` at both match-end call sites, with whatever that call's response carried under `placements`): takes the `rank: 1` entry's `name`, runs it through `game/name_snap.py::find_winning_slot()` (same fuzzy-fold + confidence-floor + ambiguity-margin caution as `NameSnapper.snap_all()`'s fuzzy pass, just resolving one name against the small already-known `_prediction_slot_map` instead of a whole bar against ladder aliases) to find the winning slot, then the outcome id for that slot. If any step comes back empty — no placements, no confident name match, no known outcome for that slot — the prediction is `CANCELED` (full refund) rather than resolved on a guess. `_post_results_to_ingest()` was extended to return the ladder's response body (previously void) specifically so `placements` could flow back to this call; `DraftLifecycle.post_results()` and `ingest.post_results_screenshot()` both had to start returning it the same way.

**`placements` already exists server-side (confirmed 2026-09-11 against the darwin-stalker source, cloned locally for reference — see `docs/DS_LIFECYCLE_HANDOFF.md`'s screenshot-endpoint section):** this used to be documented here as a field DarwinStalker still needed to add, written the same day (2026-09-09) the server actually shipped it — the note just never got updated. `src/api/drafts.rs` already returns `placements: [{rank, name, player_id}, ...]` (finish order, `name` through the ladder's own alias+fold resolver, empty `[]` rather than a guess whenever OCR failed or an ordering can't be vouched for), and the bot's `post_results_screenshot()`/`_post_results_to_ingest()` already return the whole parsed response body unfiltered, so `placements` should already be flowing through with zero bot-side changes needed. Not yet confirmed with a live `RESOLVED` (non-`CANCELED`) prediction from an actual completed match — that's the remaining thing to watch for to be fully sure it's working end-to-end, not just wired correctly on paper.

**Cleanup on abort:** `_reset_session()` (every path back to `IDLE`) cancels any still-open `_active_prediction` for a full refund — cleared synchronously so a second reset before the cancel call lands can't double-fire, with the actual Twitch call itself fired fire-and-forget like the rest of this integration's cleanup (`stop_stream()`, `_clear_streaming_presence()`).
...
- When the real reactor count reaches `scrim_min_players` (default 8), `scrim_admin_role` is pinged in the **`ai-director`** channel (ID `1520518111089000548`) — not the signup channel itself. **Cooldown, `_QUEUE_FULL_PING_COOLDOWN_SECONDS` = 10s (2026-09-12):** a player spam-reacting (un-react then react again) can bounce the count off exactly `scrim_min_players` several times in a row, re-triggering this branch every time it lands back on that exact value and paging the admins repeatedly for the same full queue. `ScrimCog._last_queue_full_ping_at` (a plain `time.monotonic()` timestamp, set only once the message actually sends — not just when the threshold check passes) caps how often the ping can go out, same shape as `DarwinTwitchBot._POV_REDEMPTION_COOLDOWN_SECONDS`. In-memory only, like the reset timer below — a bot restart clears it.
...
| `/role remove <lobby>` | **Required `lobby` choice, `1`, `2`, or `Both` (2026-09-10).** Removes the targeted lobby's role(s) (`scrim_player_role` for `1`, `scrim_player_role_2` for `2`, both for `Both`) from everyone who currently has them, then deletes and reposts the signup message **once** — see below, a single reset already clears **both** lobbies' reactions regardless of which role(s) were targeted, so `Both` exists specifically to avoid resetting the queue twice when both need clearing. Response is public (not ephemeral). |
...
3. **Current:** `_delete_and_repost_signup_message()` deletes the tracked signup message and calls `_ensure_signup_message()` to post a fresh one (reusing the same "tracked message was deleted, post a new one" path that already runs on every `on_ready`) — no per-user loop to race, and a brand-new message can't carry any stale reaction cache. Trade-off: a new message appears in the channel (with a new tracked ID, auto-persisted) every time `/role remove` runs, and — same as step 2 — this always resets **both** lobbies' signups regardless of which `lobby=` was targeted, so `role_remove()` warns the admin in its response if the other lobby still has active players (only when a single lobby was targeted — see below, the `Both` choice makes this warning moot).
...
**`/role remove`'s `Both` choice (2026-09-10):** since the reaction/signup-message reset already clears both lobbies regardless of which single role is targeted (point 3 above), running `/role remove 1` then `/role remove 2` back-to-back reset the queue **twice** for no reason — the second repost would wipe out reactions that had only just re-accumulated after the first. `lobby=Both` removes `scrim_player_role` *and* `scrim_player_role_2` (each snapshotted and processed the same way as the single-lobby path) but calls `_delete_and_repost_signup_message()` exactly once at the end, regardless of how many of the two roles actually had anyone to remove. If neither role has any members, nothing is reset at all — same "nothing changed, don't touch the queue" behavior the single-lobby path already had. The "other lobby still has active players" warning is skipped entirely when `Both` is chosen, since there's no longer an "other" lobby that wasn't targeted.
...
**Live region-breakdown message below the signup message, not a `/role add` reply (2026-09-04, moved to a standing message 2026-09-09, numbered by signup order 2026-09-11):** originally `_build_region_table()` appended a monospace table (Player / NA / EU columns, `X`/`-` per cell) to `/role add`'s response, covering only whoever that invocation had just assigned. It's now a standing message of its own — `_ensure_region_message()` posts it directly below the signup message (same channel, `scrim_signup_channel_id`) the first time both exist, and it tracks **every current signup reactor**, not just role-add's picks, so it reflects reality even before `/role add` has ever run. `_region_list_lines(guild, members)` (renamed from `_region_table_lines`) still builds the same monospace `#` / `Player` / `NA` / `EU` table with `X`/`-` per region cell — a plain numbered-list-only version was tried and reverted the same day; the `X` marker made scanning many rows at a glance easier than a trailing `"— NA"` text tag — just with a `#` column added (1-based, in the order `members` is given) and returns `None` if neither region role exists in the guild, same "nothing to show" case as before. `_region_breakdown_embed()` wraps those rows in a small embed (`_COLOR_NEUTRAL`, monospace code-block description). `/role add`'s response still doesn't include any region info of its own.

**Ordered by actual signup time, not Discord's reaction-iteration order (2026-09-11):** every call site that feeds this message now passes `_ordered_reactors(message)` instead of the plain `_reactors(message)` — `on_ready()`'s startup refresh, `_ensure_region_message()`'s first-post, and both `on_raw_reaction_add`/`on_raw_reaction_remove` (which also use the same call's result for the plain reactor `count`, unaffected by order). The numbering this produces isn't just cosmetic: since `/role add` assigns lobbies by signup order too (see below), a player's position in this list is literally which lobby they're headed for — 1-10 is lobby 1, 11-20 is lobby 2 (if `scrim_player_role_2` is configured).

**New-account/new-member flag (2026-09-11):** a discreet signal for admins vetting signups against banned players re-joining under alt Discord accounts (raised in conversation, not a hard block — a genuinely new legitimate player looks identical to this on its own). `ScrimCog._is_new_account_or_member(member)` flags a row if the account was **created** OR **joined this guild** less than `_NEW_ACCOUNT_FLAG_DAYS` (7, hardcoded — not config, since this isn't per-deployment calibration) days ago — both `discord.Member.created_at` and `.joined_at` are already present on every member object the bot handles, no extra intent or permission needed. A flagged row in `_region_list_lines()` gets a trailing `` *`` marker and is wrapped in `\x1b[0;31m...\x1b[0m` (red), with a one-line legend appended below the table only when at least one row is flagged. `_region_breakdown_embed()`'s code fence was changed from plain ` ``` ` to ` ```ansi ` so Discord's desktop/web clients render those escapes as actual color instead of literal control characters — a plain fenced block has no other way to color a single row, since the embed's own `color` field applies to the whole embed, not per-line. Clients without ANSI code-block support (some mobile clients) still show the `*` marker, just not the color.
...
**`_ensure_region_message()`'s check-then-create body is lock-protected (2026-09-10 fix, found live: a reset left a permanent duplicate message behind).** A `/role remove` reset deletes the old signup and region messages, then posts a fresh signup message (`_ensure_signup_message()`) *before* reposting the region message (`_ensure_region_message()`) — and a viewer reacting to that brand-new signup message in the gap between those two steps fires `on_raw_reaction_add` immediately, which calls `_update_region_message()`, which (seeing the same now-stale `scrim_region_message_id` the reset hasn't cleared yet) falls back to calling `_ensure_region_message()` too. Without serialization, both calls independently see "the tracked id doesn't resolve" at the same instant and both post a brand-new message — whichever's `_save_message_id()` runs last wins the tracked slot, and the other becomes a permanent orphan: nothing points to it anymore, so no future reset (or anything else) ever finds it to clean up — it just sits in the channel forever. Fixed with `ScrimCog._region_message_lock` (an `asyncio.Lock`), wrapping the entire check-existing/post-new/save-id sequence in `_ensure_region_message()` — whichever call gets there first finishes its full cycle (including the `_save_message_id()` write) before the second call's existence check even runs, so the second call correctly sees the freshly-created message and returns without posting. A lingering duplicate from before this fix has to be deleted manually — the bot has no way to discover an untracked message on its own.
...
**Points decrease by direct subtraction, not by invalidating and re-reading (2026-09-10 fix):** points only genuinely decrease when this bot itself plays a card, and every card's cost is already known (`CARD_POINT_COSTS`/`CardEvent.points_cost`) — so `_debit_points(cost)` just subtracts it straight from `self._last_confirmed_points` (clamped at 0) instead of the old approach of setting it to `None` and waiting on a fresh OCR read to re-establish a baseline. Called from `_play_tray_card()`'s tail (only when `played` is `True` — a confirmed-failed attempt spent nothing, so the known value is left untouched entirely) and unconditionally from `_attempt_zone_close()` (that path has no verification step at all, so a play is always assumed to have happened). Falls back to the old invalidate-to-`None` behavior only when the cost isn't known (a `card_type` missing from `CARD_POINT_COSTS`) or there's no confirmed baseline yet to subtract from. This means a card fired shortly after another no longer has to block on `_wait_for_points()` for a fresh read that would just re-confirm arithmetic the bot already knew.
...
**`read_director_points` had no plausibility ceiling — a single overshooting misread got permanently latched as the match's points baseline (2026-09-13 fix, found live: Beach Party played without enough points):** the function parsed `"".join(filter(str.isdigit, text))` and returned whatever integer came out, however many digits — with no check against the game's own hard cap of 10. `director_points_use_pips` is `false` in this repo's config, so `_read_points()` has no independent pip-based cross-validation to catch a bad OCR value either. Combined with `_update_points_reading()`'s ratchet-up guard (see above) — which only ever rejects a read that's *lower* than the last confirmed value, never one that's implausibly *higher* — a single corrupted OCR frame (a stray extra digit from noise/antialiasing in the 4x-upscaled crop) could overshoot to something like 179 and get accepted outright as the new confirmed baseline, since `179 >= last_confirmed` trivially holds. From there `_debit_points()`'s arithmetic is completely correct — it just subtracts each card's real cost from the fabricated baseline — so the log looked internally consistent while being built on a number the game can never actually produce. Confirmed directly in `logs/darwin_bot.log`, 2026-09-11 08:01-08:09: `Points ready for 'Beach Party at 4:00': 179/5` → `zone_close: 174/3` → `Electromania at 6:30: 171/3` → `zone_close: 168/3` → `Telepathy at 10:00: 165/3` → `zone_close: 162/3` — 179 decaying by exactly each card's cost, never correcting back toward reality. The same shape (a too-high baseline decaying only by real debits) recurred across many other matches in the log. Once this happens, every card affordability check for the rest of that match passes instantly regardless of the true on-screen value — which is exactly what "the bot played Beach Party without enough points" looks like from the outside: not a skipped check, but a check that always says yes because the confirmed value is fiction.

**Fix:** `read_director_points()` (`game/ocr.py`) now rejects any parsed value above 10 outright, returning `None` (logging the raw OCR text at debug) instead of the impossible number. An out-of-range read isn't evidence of 10 points either — clamping it to 10 would still fabricate a confirmed baseline the game never actually produced, just a smaller lie than 179. Returning `None` instead folds straight into `_update_points_reading()`'s existing "failed read — keep the last known value" path (the same one an ordinary OCR miss already takes), so a corrupted frame is disregarded entirely and the next genuine read gets a fair shot at re-establishing the true value, rather than a bad frame ever becoming a new baseline of any size. `tests/test_ocr_director_points.py` pins this down directly, including the exact 179 misread from the log above and the other observed overshoot values (106, 77, 64, 56, 53, 50, 45, 40).

**The ratchet-up guard itself was still one-sided — a smaller, in-range overshoot could still get through (2026-09-14 fix, found live: Electromania at 6:30 fired on inherited points from a bad `zone_close` read):** the fix above only rejects *impossible* values (>10), which the OCR-parsing layer can check with total confidence. It does nothing for a misread that overshoots into a still-plausible single-digit value — `_update_points_reading()`'s ratchet only ever compared *direction* (`current >= last_confirmed_points` → accept), never how large the jump was. Confirmed live in `logs/darwin_bot.log`: `have 2` held steady across six 2-second polls, then the very next read jumped straight to `Points ready for 'zone_close': 7/3` — a +5 jump in one 2-second interval, far outside the established regen pace, and still ≤10 so the OCR-layer fix above never saw anything wrong with it. `zone_close` debited that `7` down to `4` (`7 - 3 = 4`, exact arithmetic), and `Electromania at 6:30` inherited that `4` three seconds later with no fresh read of its own — the fabricated value had already cleared its 3-point threshold the instant the card was due.

**First fix, same day:** `_update_points_reading()`'s ratchet was bounded on *both* sides, not just the low side — a read was only trusted if it landed within `[last_confirmed, last_confirmed + 2]`; anything below that range *or* above it was disregarded as a misread. A separate stuck-timeout (30s with no trusted update) existed as an escape hatch, since bounding acceptance to such a narrow window meant a confirmed value that was already wrong would otherwise reject every subsequent *correct* read too, forever.

**That fix didn't survive contact with real background-sampling gaps (replaced within the hour, 2026-09-14):** a flat "+2, always" cap implicitly assumes every read is spaced the same distance apart. It isn't — `_wait_for_points()` polls every 2s, but the main loop's background sampler only polls every `screen_poll_interval_seconds` (12s default), and a poll can silently fail (OCR miss) and widen the *effective* gap further still. Real regen over a 12s+ gap routinely exceeds 2 points, so a perfectly legitimate jump got rejected — which left the baseline stale, which made the *next* read's required jump even bigger, guaranteeing rejection again. In practice this meant nearly every background-poll update was routing through the 30s stuck-timeout instead of ever landing cleanly: five separate stuck-timeout escapes inside 40 minutes of one live match, e.g. `Points read 10 (background poll) is outside the trusted range of last known 4 (+0 to +2)... accepting it as a fresh baseline` — a +6 jump that was completely real, just took 30 seconds longer than it should have to be believed.

**Second fix, same day: trust requires a confirming second read, not a bounded jump.** A read that disagrees with the confirmed value — higher *or* lower, no asymmetry — was never trusted on the strength of one frame alone: `_update_points_reading()` immediately took a second, independently-captured read (`self._read_points(take_screenshot())`) right then, accepting the change only if it landed on that exact same value. This needs no assumption about the true regen rate at all — unlike a flat jump cap, it doesn't care how big the jump is or how long it's been — and is symmetric by construction, so it subsumed the stuck-timeout's whole job too: a wrong baseline self-corrects the moment two reads agree, in either direction, with no separate escape hatch needed.

**That forced re-read had the exact same failure mode it was built to avoid (revised within the hour, same day):** the confirmation attempt is itself just another OCR read — equally capable of coming back `None` as any other one. A `None` confirmation was being compared with `!=` against `current`, which is `True` for `None != 7`, so a whiffed confirmation frame silently rejected a perfectly good `current` read for no reason but its own bad luck. This is the exact "using the null reads" failure the design was supposed to route around, just moved one step later.

**Current fix: compare against the last *valid* read, not a freshly-forced one.** `self._last_valid_points_read` tracks the most recent non-`None` read regardless of source (background poll or `_wait_for_points()`'s loop) or whether it was ever confirmed. A disagreeing `current` is accepted once it equals `self._last_valid_points_read` — the previous call's real read, whenever it happened, not a read manufactured on the spot. A `None` read never updates this field and never participates in the comparison on either side, so it can neither falsely confirm nor falsely deny anything. Whether accepted or not, `current` always becomes the new `_last_valid_points_read` at the end — a rejected read still becomes the candidate the *next* real read gets checked against, so agreement is found whenever it naturally occurs (the next background poll, or the next `_wait_for_points()` tick a couple seconds later), without needing to force an extra screenshot+OCR call at all. Costs nothing extra over a plain read either way. `tests/test_match_runner_points_ratchet.py` covers the agreement/disagreement shape (including agreement arriving several calls later, past an intervening failed read) and specifically pins down the null-confirmation bug this revision fixes.
...
**Mouse clicks need it too, and most of `bot/discord_bot.py`'s click flows weren't calling it (2026-09-13 fix, found live: the post-match MAIN MENU click missed):** every `pyautogui.click()`-based automation flow in `discord_bot.py` (`_do_launch()`'s splash-screen Continue, `_do_create_custom()`'s whole custom-lobby sequence, `_do_go_to_menu()`'s navigation loop, `_do_post_match_return()`'s results-screen MAIN MENU click) needs Darwin to actually be the OS-focused window for a click to register in-game — same underlying requirement `press_key()`'s docstring already describes for keyboard input, just never applied to mouse clicks here. Only `_do_post_match_return()` called `focus_darwin_window()` at all, and even there the return value was discarded — a failed focus grab (Windows' foreground-lock timeout, or the hwnd not being ready yet) went completely unnoticed, and the click proceeded anyway wherever the cursor happened to land, which can miss entirely if something else (Discord, the console) still held focus. This matters most right after a match ends, since nothing forces Darwin back to the foreground between the match itself and this click — whatever the admin was last interacting with (most likely Discord) may still be focused.

**Fix:** all four functions now call `focus_darwin_window()` (via `from game.card_actions import focus_darwin_window`) before their click sequence begins, check the return value, and retry once after a short pause on failure — logging a warning either way rather than silently proceeding blind. None of this is treated as a hard failure on its own (each flow's existing template-based verification/retry — `click_until`, `wait_for_template_center` timeouts — is still the actual safety net that catches a click that didn't land), but a focus failure is now visible in the log instead of indistinguishable from any other cause of a missed click.
...
- Wiring: `DirectorCog.custom` → `_open_ds_draft()` right after the `IN_CUSTOM` transition; `MatchRunner.run()` → `_fire_player_bar_init()`, which detects the roster then calls `on_match_start_async(self._player_names)` (empty OCR = WARNING, not a silent skip — that silent skip is why the Twitch embed never lit for a week), both together on their own thread (see below); `_post_results_to_ingest` → `post_results`; `_reset_session(reason)` → `_close_ds_draft(reason)`.
...
- **`_init_player_bar()` itself joined this fire-and-forget group too (2026-09-10 fix, found in review):** the 2026-09-07 fix above moved the ladder roster *push* off the match thread, but `_init_player_bar()` — the detection + OCR call that produces the names being pushed — was still called inline, directly before it, and can do up to one tesseract call per card (`game/player_cards_v2.py::ocr_names`, each individually timeout-guarded up to 10s in `game/ocr.py`) — by far the most expensive of the pre-B-press steps, yet never fixed alongside its siblings. `MatchRunner.run()` now calls `_fire_player_bar_init()` instead of `_init_player_bar()` directly: it spawns a daemon thread running `_init_player_bar_and_push()`, which does the detection *and* both ladder pushes (`on_match_start_async` + the `match_start` event) together, in that order, since the pushes need the names detection just read. `self._player_slot_xs`/`_player_names`/`_player_alive` stay at their empty defaults until that thread finishes — `_poll_player_bar()` already no-ops gracefully on an empty `_player_slot_xs`, so a match's first poll or two simply detects nothing rather than blocking anything waiting on it. Covered by `tests/test_match_runner_player_bar_init.py`.
- Live match events (2026-09-06): `MatchRunner` emits `match_start`, `first_blood`, `eliminated` (the player bar is now polled ALL match, not just until first blood), `match_end`, `card_play`, `status`, `aborted`; `/say` emits `say`. Also, since 2026-09-09, `slot_map` (diagnostic — see docs/PLAYER_BAR_CALIBRATION.md §8), and since 2026-09-10, `feed_first_blood` (diagnostic — see Damage feed OCR below) — full field list in docs/DS_LIFECYCLE_HANDOFF.md §3a. All go through `DraftLifecycle.event()` → `game/ds_relay.py` (daemon thread, batched POSTs to `/api/ingest/events`, never blocks the match loop). Game index: seeded from the server's own `next_game_index` (open-draft's reply, added 2026-09-11) when the draft id actually changes at `open_lobby`, falling back to `1` if the reply doesn't carry one — previously always `1` regardless, which left the bot's local counter wrong after a restart mid-set (the server already floors its own event attribution against this, but the bot's own value stayed stale). +1 after each `post_results` (which flushes first), 0 at `close` (which flushes first). See docs/DS_LIFECYCLE_HANDOFF.md's "The bot's own game counter" note.

**Damage feed OCR — "who got the kill" (2026-09-10):** `MatchRunner._poll_damage_feed()` OCRs the in-game damage/kill feed text every poll, checking each line against `_FEED_PATTERNS` (a module-level `[(event_kind, compiled_regex), ...]` list — `feed_first_blood` (`"X DREW FIRST BLOOD FROM Y"`) and `feed_kill` (`"X KILLED Y BY Z"`, `Z` optional — see below), both in all caps; add more entries there for future keyword matches, no other code changes needed). This is a distinct capability from `first_blood`/`eliminated` above, which come from the player-bar alive/dead flip and can only tell you WHO died — this is the only signal that tells you WHO got the kill, and now how. Uses `game/ocr.py::ocr_feed_text()` on `game.video_recorder._CROP_REGION` — the same crop already used for match recordings, not a separately calibrated region — with `--psm 6` (multi-line block) since the feed stacks several recent lines in that space. This replaces an older single-line version, `ocr_kill_notification()`/`kill_notification_region` — removed entirely 2026-09-10 (not just its call site), since it was never calibrated on any machine, its one call site (`_poll_player_bar()`'s first-blood branch) was inline/blocking on the match thread (the exact risk this feature was built to avoid), and this function already covers the same need on an already-calibrated region. The OCR'd text is matched **per physical line**, not flattened to one string first — flattening before matching was tried and found (via a unit test, not a real match) to let an adjacent unrelated feed line's words bleed into a pattern's captured groups, since a non-anchored regex just eats everything up to/after the phrase across whatever "line" it's searching. A pattern's named groups `killer`/`victim` (if present) are resolved against `self._player_names` (the match-start roster) via `MatchRunner._resolve_feed_name()` (see below) — so a low-confidence, ambiguous, or genuinely-not-in-this-roster OCR read resolves to `None` rather than a guess; a `method` group (no name resolution — it isn't a player) rides along as plain text if present. A pattern with no such groups still emits, just without the corresponding fields. Runs for the whole match, every poll — nothing here assumes "only fires once," since kills in particular are expected to recur many times. Every pattern just emits its `event_kind` — `feed_first_blood` is the one exception, additionally queuing the first-blood reward below once its killer resolves confidently.

**The give_wood reward went to the wrong player — a clean OCR read matched an unrelated roster name (2026-09-11 fix):** the feed clearly read `"CONNOR DREW FIRST BLOOD FROM GZ"` — no OCR garbling, `killer_raw` was literally `"CONNOR"` — but the real "Connor" wasn't in that match's roster under that display name at all (their nameplate had been ladder-linked to a different display name, "Mojo", for this match). `find_winning_slot("CONNOR", slot_map)` — comparing only against each slot's single already-decided display name — still scored a completely unrelated player, "Coen", at exactly `0.600` (precisely `_MIN_FUZZY_CONFIDENCE`'s old floor, next-best candidate only `0.5`), so the ambiguity-margin check never caught it either; the reward went to Coen instead of nobody.

Two fixes, in priority order:
1. **`killer`/`victim` resolution tries the ladder's own known-alias data first, via the new `DraftLifecycle.resolve_alias(name)` (`game/ds_lifecycle.py`) — an exact fold match against a player's FULL set of registered handles (canonical name, persona, every alias in `names`), not just their one already-decided slot display name.** "connor" was one of that player's other known ladder aliases the whole time (it's exactly what their nameplate OCR had snapped from at match start) — `resolve_alias("CONNOR")` finds that directly, no scoring involved, so it can't misfire the way a fuzzy comparison can. Deliberately reuses `NameSnapper.snap()`'s single-read exact-or-glued-substring logic, never `snap_all()`'s fuzzy multi-round pass — that pass exists for "claim each of many reads across a whole bar," not a one-off lookup, and a fuzzy score against an unrelated player is exactly the failure being avoided here. `MatchRunner._resolve_feed_name(raw_name, slot_map)` is the new call site (replacing a direct `find_winning_slot()` call in `_poll_damage_feed_worker()`): tries `resolve_alias()` first, and only falls back to `find_winning_slot()`'s fuzzy match against `slot_map` when no alias resolves (an unlinked player with no registered aliases, genuine OCR noise, or `self._ds` being `None`) — same behavior as before this method existed for all of those cases.
2. **`_MIN_FUZZY_CONFIDENCE` raised `0.6` → `0.65` as defense-in-depth for that fallback path** — every legitimate fuzzy match in this project's own tests (a genuinely truncated or lightly-misread name) scores `0.714`-`0.903`, comfortably clear of the new floor, so this closes the specific "Connor"/"Coen" gap (0.600) without touching real matches, for whenever alias resolution itself can't help (no ladder data available). Shared constant, so it also tightens `NameSnapper.snap_all()`'s player-bar name-snapping pass and the results-screen "who wins" resolution, not just the damage feed.

See `tests/test_ds_lifecycle.py`'s `resolve_alias()` tests and `tests/test_match_runner_feed.py::test_alias_resolution_wins_over_the_fuzzy_fallback` (built from this match's exact roster) for the fix's coverage; `tests/test_name_snap.py::test_find_winning_slot_refuses_a_clean_read_that_matches_no_one` for the floor-bump fallback.

**Every kill was reaching the ladder 2-4 times, and freezing to death counted as a kill (2026-09-20 fix, from an audit of darwinstalker.com's stored `match_events`; full write-up in docs/FEED_DEDUPE_HANDOFF.md):** `_recent_feed_matches` keyed on the exact OCR'd line, but one feed line stays on screen for several polls and OCRs a little differently each time (`BY AXE` / `BY AXE,` / `. X KILLED...`), so the key almost never matched its own repeat: 222 of 867 resolved kills on prod were repeats and first blood averaged 1.7 events per game. `_poll_damage_feed_worker()` now also records a once-per-match key: `once|died|<victim_slot>` (a player dies once per match) and `once|feed_first_blood`. It is recorded **only for a complete read** (`victim_slot` resolved plus `killer_slot` or a `cause`; for first blood, `killer_slot` resolved), so a garbled first frame never blocks the good read that follows it, and never blocks the first-blood reward, which needs `killer_slot`. Incomplete reads still fall back to the text key; the ladder dedupes whatever gets through. Separately, `"X KILLED BY COLD"` has no second name, so the `feed_kill` regex captured `victim="BY COLD"` and the ladder credited X with a kill. `_split_by_cause()` now reads that tail: a weapon word (`_WEAPON_CAUSES`: AXE/ARROW/HEADSHOT, fused `BYARROW` included) is a real kill whose victim the OCR lost (killer kept, no victim fields, tail becomes `method`); any other cause is X dying to the environment and is emitted as `victim_raw`/`victim_slot` + `cause`, with no `killer_*` fields at all. A name that merely starts with "By" (no space, not a weapon) is left alone. `_RECENT_FEED_MATCHES_MAXLEN` went 50 → 200 since the deque now holds both key types. Tests: the `once-per-match dedupe + environmental deaths` block at the bottom of `tests/test_match_runner_feed.py`.

**Two fixes from a live miss (2026-09-10): a real first blood went completely undetected, no reward given.** `_poll_player_bar()`'s own alive/dead flip caught the kill fine (it logged `FIRST BLOOD — victim: X`) — but no `feed_first_blood` match ever appeared in the log, before or after. A live screenshot taken mid-match while a match was ongoing confirmed why: the damage-feed crop showed **no banner at all**, just game-world background — kill-feed banners are transient, visible for only a few seconds, then gone for good. Two separate problems, found from the same incident:

1. **Poll cadence was too coarse.** `_poll_damage_feed()` was called every main-loop iteration, but the loop's own sleep was capped at `screen_poll_interval_seconds` (12s by default) whenever no card was due soon — so during a long gap between cards, the damage feed was effectively checked only once every ~12 seconds. First blood is a one-time event with exactly one banner; if no poll happens to land inside its few-second visible window, it's gone for good, no later poll can retroactively catch it. Fixed by `_MAIN_LOOP_MAX_SLEEP_SECONDS` (3s) — a hard ceiling on the main loop's own iteration cadence, independent of `screen_poll_interval_seconds` (which still governs the separately-gated director-points sampling, unaffected by this change). Every other per-iteration check was already either independently time-gated or cheap enough to run more often for free (`_poll_player_bar`, `_match_has_ended()`, card-fire timing), so this only meaningfully speeds up the one thing that actually needed it.
2. **`ocr_feed_text()`'s preprocessing didn't handle colored player names.** Two `feed_kill` matches that *did* fire around the same live match show the actual gap plainly: the white action words ("KILLED", "BY", "HEADSHOT") read fine, but the player names came back as pure garbage (`"~~"`, `"ww ly"`, `killer_slot`/`victim_slot` both `None`). The function converted the crop to grayscale via `cv2.COLOR_BGR2GRAY` (a weighted-luminance formula) before thresholding — a saturated colored name can have low luminance despite looking bright in color, so this conversion doesn't reliably separate colored text from a colored background. `game/player_cards_v2.py::ocr_prepare()` already solved this exact problem for player-bar names with a per-pixel **MIN across B/G/R** instead of a weighted grayscale (white text has every channel high, so the min stays high; a saturated color has at least one channel low) — `ocr_feed_text()` now uses the same trick (upscale 3x → per-pixel min → invert), dropping the old fixed-threshold binarization entirely.

**Still not fixed after the above (2026-09-10, found live the same day): player names in this feed are orange/red, not white.** The min-channel trick above only separates white text from a colored background — an orange/red pixel's *minimum* channel is its blue channel, which is low the same way a saturated colored background's low channel is, so this preprocessing can't tell orange/red text apart from the background at all; the name portion still vanishes or turns to noise before tesseract sees it, even with the fix above in place. Rather than guess at a color threshold with no ground truth to test it against, `game/ocr.py::save_feed_debug_images()` was added to capture the raw natural-color crop (plus the processed image tesseract actually received) to `screenshots/errors/` as `{timestamp}_{kind}_raw.png` / `{timestamp}_{kind}_processed.png`, called from `MatchRunner._save_feed_debug_images()` in `_poll_damage_feed_worker()` every time a feed pattern matches.

**Fixed for real, same day, once two live captures came in:** the next first blood produced two raw crops (kept as `tests/fixtures/feed_first_blood/`, see its README) that made the actual rendering plain — action words are white, and every player name in the feed renders in the same one fixed orange/red color (not per-player — it's the same color for everyone) — and, critically, **every glyph in this font, regardless of fill color, is drawn with the same dark near-black outline stroke** (measured directly off the fixtures: outline BGR ~(28,15,0), max channel ≤ ~30-75 even on an anti-aliased edge; both real backgrounds sampled well above that — wood ~130-150, snow much higher — and both fill colors, white and colored, sampled well above it too). So `ocr_feed_text()`'s preprocessing (`_prepare_feed_crop()` in `game/ocr.py`) was rewritten a second time to detect the **outline**, not the fill: any pixel with `max(B,G,R)` below `_FEED_TEXT_DARKNESS_THRESHOLD` (60, chosen from a 40-90 range that all gave identical clean results against both fixtures) is "ink," everything else — background *or* either fill color — is background. This is color-blind by construction, so it would keep working even if the name color ever changed. Verified directly against pytesseract on both fixtures: the old min-channel approach read pure garbage for both names (`"~~~) ... é"`, `": ... :"` — regex captured `killer`/`victim` groups that never resolved to a slot); the outline-based rewrite reads `"PEFISS"` and `"TWO"` correctly in both. `tests/test_ocr_feed_colors.py` is a hard regression guard built directly from these two fixtures, not just a smoke test.

**One more real-world gap surfaced immediately after, from six live `feed_kill` captures in the same match:** OCR text was now fully legible — but two of the six still didn't match `_FEED_PATTERNS`, even with every word reading correctly, because the game renders near-zero pixel gap between a colored player name and an adjacent white word (a normal gap between two white words is preserved) — e.g. `"TWO KILLED THUGZ BY ARROW"` OCR'd as `"TWOKILLEDTHUGZBY ARROW"`: no space around `KILLED` (that boundary touches the colored name), but the space before `ARROW` survived (white-to-white). `_FEED_PATTERNS`' keyword boundaries required `\s+`, which silently failed to match legible text. Both patterns now use `\s*` instead — verified against the two real captures (kept as `tests/fixtures/feed_kill/`, see its README) that failed under `\s+`; `tests/test_match_runner_feed.py`'s two new end-to-end tests drive the real `ocr_feed_text()` (not mocked) against them to prove the whole pipeline resolves a merged-word line, not just the regex in isolation. The other four captures from that batch didn't get fixed by this: one had no real kill line in that frame at all (the pre-rewrite OCR's "KILLED" match on it turned out to be pure noise, a false positive that the outline rewrite alone eliminated), and two had "KILLED" itself misread badly enough (`"WONLLED"`, `"KLLED"`) that no reasonable regex tolerance would catch them without risking false matches elsewhere — left as a known remaining gap, not chased further on this little evidence.

**`feed_kill` — every kill, not just the first (2026-09-10):** matches `"X KILLED Y BY Z"` (`Z` — the method: axe, arrow, cold, etc. — is an optional trailing group, so a line missing it, whether genuinely method-less or OCR clipping the tail, still matches with no `method` field rather than not matching at all). Emits `killer_raw`/`killer_slot`, `victim_raw`/`victim_slot`, `method` (when present), and `text` — same resolution as `feed_first_blood`, but drives no reward/action logic; this one is purely for the ladder's live feed. Deliberately does **not** share any state with the first-blood reward, even though both patterns can independently match a killer/victim — `feed_kill` never touches `self._first_blood_reward_slot`.

**Duplicate-emission guard (2026-09-10):** since most patterns (unlike first blood) recur many times a match, `self._recent_feed_matches` (a `deque(maxlen=50)` of `"{kind}|{line}"` strings) skips re-emitting the exact same (kind, matched line) if it's still visible on a later poll — the feed line can plausibly linger across more than one `screen_poll_interval_seconds`-spaced poll. Bounded rather than a growing set, since a real match realistically has at most a few dozen kills. Only ever touched from the feed-poll's own background thread (never concurrently, gated by `self._feed_poll_busy`), so no lock needed.

**Fire-and-forget on its own thread, not inline in the match loop (2026-09-10):** `game/ocr.py`'s own `_OCR_TIMEOUT_SECONDS = 10` is an admission that a single tesseract call can occasionally take real time — running the feed OCR inline (an earlier draft did, sharing `_poll_player_bar`'s screenshot to save a capture) meant a slow call could delay a scheduled card play by however long it took, since the main loop's next card-fire check only happens after whatever synchronous work the current iteration is doing finishes. `_poll_damage_feed()` now only decides whether to start a poll — `self._feed_poll_busy` (plain bool, no lock, same cross-thread-flag convention as `self._lobby_captured` etc.) skips starting a new one while one is still in flight — and `_poll_damage_feed_worker()`, doing the actual screenshot + OCR + matching, runs on its own daemon thread, same pattern as `_fire_slot_map_snapshot()` and for the same reason. `_poll_player_bar()` (pixel/color sampling, no OCR) stays inline — it's fast enough not to need this.

**First-blood reward — "Give Wood" to whoever got the kill (2026-09-10):** once `_poll_damage_feed_worker()` resolves a `feed_first_blood` killer with confidence, `_maybe_queue_first_blood_reward(killer_slot)` records that card index (`self._first_blood_reward_slot`) — plain attribute write from the feed-poll's own background thread, same cross-thread-flag convention as everywhere else in this class. Nothing fires from there directly; the main loop's `_maybe_fire_first_blood_reward(elapsed, card_schedule)`, called every iteration right after the points sample, is the only thing that ever acts on it, and only once **all** of these hold:
1. **The target is still alive** — checked against `self._player_alive`, already refreshed this same iteration by `_poll_player_bar()` (no extra screenshot). A kill on the intended recipient before the reward fires cancels it outright, no partial/late attempt.
2. **No real scheduled card is due within `_BONUS_CARD_TIMING_BUFFER_SECONDS` (5s, shared with the Crowd Favorite reward below)** — the reward's own drag (~1-2s, the same `_play_tray_card()` every scheduled card goes through) must never overlap a scheduled card's own drag. If one's imminent, this iteration just no-ops and tries again next iteration — the reward stays queued, nothing is lost.
3. **Enough director points** — checked against `self._last_confirmed_points`, the same continuously-sampled value the main loop already maintains; no fresh OCR read of its own. `give_wood` costs 1 (the cheapest card in the game), so this is rarely the blocker in practice.

Once clear, `self._first_blood_reward_resolved` is set **before** calling `_give_first_blood_reward()` (which blocks for the drag) so a later iteration can never re-enter and double-fire while it's in progress — there is only one first blood per match, so only one reward is ever given, canceled, or skipped (deck has no `give_wood` left), never retried after any of those outcomes.

`_give_first_blood_reward(killer_index, deck_pos)` is a thin wrapper around the shared `_give_reward_card()` (extracted 2026-09-10 when the Crowd Favorite reward below was added, since both need the exact same mechanics): drops the card directly on the target's own portrait via `_player_portrait_target()` — no POV switch, no POV lock (see the Player-Targeted Cards section below for the full 2026-09-14 rewrite, including why the original switch-POV-then-drop-at-center mechanic was replaced). Builds a synthetic `CardEvent` to reuse `_play_tray_card()` (the exact same drag/verify/retry/announce logic every scheduled card goes through) rather than duplicating it.
...
- Tournament mode (2026-09-12): `_open_ds_draft` skips opening a draft entirely while `tournament_mode` is on — see the Discord Bot section's `/tournament` note above for the full reversal (this used to tag every draft with a ladder tournament slug; that's gone, no draft at all now).
...
**"Game N" banner (2026-09-11):** an OBS text source (`obs_game_number_source` config, default `"Game Number"` — a "Text (GDI+)"/"Text (FreeType 2)" source, font/color/position set up manually in OBS same as the minimap cover) shows viewers which game in the set is currently being played. `game/obs_control.py::set_source_text(source_name, text)` pushes new text via `SetInputSettings(..., overlay=True)` — merges just the `text` field onto the source's existing settings, so it never touches font/position. No "of 4" suffix: a tournament lobby's set can be capped at fewer games server-side (`set_submissions.game_limit`), and the bot has no reliable way to know that limit, so claiming a total would risk being wrong — just "Game N".
- **Set in two places**, both reading `DraftLifecycle.game_index`: `discord_bot.py`'s `/custom` success path, right after `_open_ds_draft()` (which already seeds `game_index` — see the Ladder Ingestion section's `next_game_index` note), so the banner is correct as soon as the lobby exists; and `MatchRunner._update_game_number_banner()`, called from `_init_player_bar_and_push()` at every `/start` (`game_index` is refreshed there too, since `on_match_start()`'s own open-draft re-call carries a fresh `next_game_index` every time). In this bot's actual usage every game gets its own fresh `/custom` (not one lobby replayed across a whole set), so in practice these two call sites both fire, back to back, for every single game — the second one isn't just a "multi-game set" fallback.
- **`_init_player_bar_and_push()` now calls `DraftLifecycle.on_match_start()` directly instead of the `_async` fire-and-forget wrapper** — that wrapper exists specifically to protect the *match* thread, which this method is already off of (it runs on its own `PlayerBarInit` thread, see the Ladder Ingestion section), so blocking here for the same network call is harmless and guarantees `self._ds.game_index` is fresh before the banner update runs, instead of racing an in-flight call on a third thread.
- **Cleared at match end (2026-09-11):** `DirectorCog._clear_game_number_banner()` blanks the text (`set_source_text(source, "")`), fired from both match-end call sites (the normal `/start` completion and the auto-start watcher) right alongside `_twitch_start_ad_break()`. Since every game gets a fresh `/custom`, there's nothing left to justify the last game's number staying up once a match ends and the bot returns to the main menu awaiting the next one — without this it would just sit there, stale, through the whole idle stretch.
- Gated the same way as the rest of this section: no-ops entirely if `obs_stream_enabled` is `false`, or if there's no draft lifecycle wired up (`self._ds is None`).

**"Current points" display (2026-09-14):** an OBS text source (`obs_points_source` config, no default — empty/unset means opt-in and inert, unlike `obs_game_number_source`'s `"Game Number"` default, since this needs a source the streamer sets up manually first) shows the bot's live `self._last_confirmed_points` value, for tracking what the bot currently believes its director-points balance is without tailing the log — useful for spotting a misread live rather than only after the fact in a log grep. `MatchRunner._update_points_display()` pushes `f"{value}/10"` (or `"?/10"` while nothing's confirmed yet) via the same `set_source_text()` the Game N banner uses.
- **Only pushed on an actual value change**, not on every poll — called from `_update_points_reading()` (when a read is actually trusted and differs from the current value) and `_debit_points()` (when a real debit changes it; a 0-cost `favorite_player` debit, or a debit into an already-`None` baseline, correctly pushes nothing). This matters beyond avoiding redundant OBS calls: `_update_points_reading()` runs inside `_wait_for_points()`'s ~2s poll loop, a genuinely timing-sensitive path (see the Match Runner section's card-firing precision) — a websocket round trip on every single poll, most of which don't change anything, would add real, avoidable latency there.
- **Cleared at match end** — `run()`'s `finally` block (same one that always finalizes the video recorder, regardless of how the match ended: normal completion, force-stop, or an exception) resets `self._last_confirmed_points` to `None` and pushes the resulting `"?/10"` placeholder, so a stale number doesn't sit on screen through the idle stretch before the next match.
- Gated the same way as the rest of this section: no-ops entirely if `obs_stream_enabled` is `false` or `obs_points_source` isn't configured.

**Discord presence mirrors live status (2026-09-04):** `DirectorCog._set_streaming_presence()` switches the Discord bot's own presence to `discord.Streaming(name=..., url="https://twitch.tv/<channel>")` — the special purple "Streaming" indicator Discord clients only show for an activity whose `url` matches a recognized Twitch/YouTube watch URL (a plain custom status text does not trigger it). Called right after `/custom`'s `obs_control.start_stream()` call confirms the stream is actually live (only when `stream_started` is `True`, the one stream-start path there is now — see `/tournament`'s note above for why the tournament-mode-only delayed path this used to also cover was removed) — never called speculatively before Twitch confirms the stream is up. `_clear_streaming_presence()` (revert to no activity) is called from `_reset_session()` via `asyncio.ensure_future` (that method is sync, called from many places — same reasoning as `stop_stream()` being called directly there), so presence reverts on every path back to `IDLE`, matching the stream's own stop lifecycle exactly. Reuses `ds_ingest_twitch_channel` (config) for the channel name rather than adding a second key for the same fact — no-ops if that key isn't set. Both presence functions are best-effort and never raise.
...
```json
{
    ...
    "advanced_cards": true,               // false = disables the give_wood/Crowd Favorite bonus-card rewards (see Match Runner);
                                          // feed_first_blood/feed_kill event logging to the ladder stays on either way — see below
    ...
    // player_target_coordinates removed from config entirely (2026-09-14) — superseded
    // by _player_portrait_target(), which targets a player's own portrait via the same
    // live geometry (game/player_cards_v2.py) already used for name/alive polling, no
    // calibration needed. See Player-Targeted Cards below.
    //
    // zone_color_thresholds is still omitted entirely rather than kept as a null
    // placeholder (2026-09-07) — read via `.get(key, default)` with a fallback that
    // already matches "uncalibrated" (`{}`), so an absent key behaves identically to an
    // explicit null. Still genuinely uncalibrated, still gates a real (if currently
    // inert) feature — set it again if it's ever calibrated:
    ...
    // ladder's live feed ALL start here — driven entirely by game/player_cards_v2.py's
    // geometry detector, no config keys or per-machine calibration needed at 1920×1080.
    // See docs/PLAYER_BAR_CALIBRATION.md. An older separator-scan detector (with six
    // config keys: player_bar_detector, player_bar_region, player_separator_threshold,
    // player_portrait_y_in_bar, player_saturation_threshold, player_name_y_in_bar,
    // player_name_h) was removed 2026-09-09 — it needed player_bar_region calibrated
    // and never was on any deployed machine, so it had been dead code (silently
    // disabled) since it was added.
    // kill_notification_region removed 2026-09-10 along with the function that
    // read it (ocr_kill_notification()) — never calibrated on any machine, and
    // superseded by ocr_feed_text() on the already-calibrated video-recording
    // crop region (see Damage feed OCR in the Match Runner section above).
    ...
    "ds_ingest_tournament_slug": "",      // unused (2026-09-12) — /tournament no longer has a slug parameter, and
                                          // no draft is opened at all while tournament_mode is true. Left in
                                          // config rather than requiring a migration; harmless either way.
    ...
    "obs_game_number_source": "Game Number", // OBS text source updated to "Game N" at lobby creation and every match start
    "obs_points_source": ""   // OBS text source updated live with "N/10" (the bot's current confirmed director-points
                              // reading) whenever it changes — opt-in, no default; set this once you've created the
                              // matching OBS text source. See the OBS Twitch Streaming section's "Current points" note
}
```
...
- [ ] ~~`zone_close_card_slot`~~ / `zone_color_thresholds` — never calibrated; only needed if `_attempt_zone_close_legacy()` is ever restored. `zone_color_thresholds` (and `recording_api_endpoint`) are omitted from config.json entirely rather than kept as null placeholders (2026-09-07) — see Config Reference above for how to set any of them again
- [x] Player-targeted card drop coordinates — no calibration needed, computed live from `game/player_cards_v2.py`'s geometry (see Player-Targeted Cards below); `player_target_coordinates` (config) is gone
...
- [x] Player bar tracking — `game/player_cards_v2.py`'s geometry detector, no calibration needed at 1920×1080 (the separator-scan detector that did need `player_bar_region` + five other keys was removed 2026-09-09, never having been calibrated on any machine). Sanity-check anytime with `calibrate_player_bar.py` / F8 — docs/PLAYER_BAR_CALIBRATION.md
...
Cards that target a specific player (`expose`, `favorite_player`, `give_leather`, `give_wood`, `man_hunt`, `speed_boost`, `warm_up`) drag directly onto that player's own portrait in the player bar at the top of the screen. These are grouped as `_PLAYER_TARGETED_CARDS` in `match_runner.py`.

**No calibration needed, no camera POV switch involved (2026-09-14 rewrite).** `MatchRunner._player_portrait_target(player_index)` computes the drop coordinate as `(self._player_slot_xs[player_index], _PLAYER_PORTRAIT_TARGET_Y)` — the x comes from `game/player_cards_v2.py`'s geometry detector (the same live data already used for name OCR and alive/dead polling, see the Ladder Ingestion section's damage-feed coverage for how that data is populated), the y is a fixed constant (90, the vertical centre of the detector's `player_cards_portrait_rows`) that doesn't vary with player count — only the x-centering does, which `player_cards_v2.py` already handles per-lobby (see below). Returns `None` if `player_index` is out of range of what's currently tracked (the player-bar snapshot failed or hasn't run yet), so a caller skips the play rather than dropping blind.

**Replaced the previous switch-POV-then-drop-at-center mechanic (found live: an occasional silent miss).** The reward flows (`_give_reward_card()`, used by first blood's give_wood and Crowd Favorite's favorite_player — see the Ladder Ingestion section below) used to press the target's `/pov` digit, wait `_POV_SWITCH_SETTLE_SECONDS` (0.3s, never a measured value) for the spectate switch to land, then drop the card at screen-center (960, 540) — the game applies a "give X to player" card to whoever's currently spectated at drop time. Found live: when the switch hadn't actually landed by drop time (or the wrong player was still centered for some other reason), the card silently landed on whoever the camera was still on instead of the intended target — with `verify_card_plays: false` (this repo's setting), nothing caught it; the log showed a normal "played successfully" either way. Dropping directly on the target's own portrait removes the race entirely — no dependency on a POV switch landing in time, since the target's slot position doesn't depend on camera focus at all. `_give_reward_card()` no longer presses any POV key, waits on anything, or locks POV (`SessionState.lock_pov()`/`unlock_pov()` — still used elsewhere, see `/pov`'s match-start lock note above — this call site was the only one removed): the old lock existed specifically to stop a viewer's own `/pov` from racing the camera switch mid-sequence, and with no camera switch left to race, there's nothing left to protect.

**One known, untested wrinkle:** a player's own portrait renders enlarged while they're currently spectated (confirmed live 2026-09-14). If a reward's target happens to already be spectated for an unrelated reason (the match-start default POV-1, a viewer's own concurrent `/pov`) at the exact moment the drop lands, it's landing on a differently-sized/positioned box than `_PLAYER_PORTRAIT_TARGET_Y` assumes for the normal case. Not expected to cause a miss in practice — the enlargement is expected to grow around the same horizontal centre, so the fixed x should usually still land inside it — but this hasn't been confirmed against a live enlarged-portrait screenshot. Watch for it in practice before assuming it's a non-issue.

**The generic scheduled-card dispatch (`_fire_card_event()`'s `_PLAYER_TARGETED_CARDS` branch) uses the same mechanism**, picking a random currently-alive player's index (`self._player_alive`) and resolving their coordinate the same way — replacing the old `player_target_coordinates` config lookup, which was never calibrated on any machine and always logged "skipping." No profile currently schedules a card in this set (only `give_wood`/`favorite_player` ever actually fire, both through `_give_reward_card()`'s reward flows, not this schedule path) — this exists so a future profile that does works out of the box, with no config to fill in first.

**Player bar layout (background, from `game/player_cards_v2.py` — see also docs/PLAYER_BAR_CALIBRATION.md):**
- 2–10 numbered player cards run across the top of the HUD, re-centred around screen-x 960 at a constant ~131.6px pitch — the detector tries every candidate count in that range and trusts the largest one where every computed position reads as a real card, so a lobby with fewer than 10 players is handled natively, not a special case.
- A 10th card sometimes appears at the far left — a known client glitch (spinning loading icon, no name, no health bar in its blank form; confirmed live to sometimes be a full duplicate of a real player's card instead). It's structurally excluded from every read: it always sits exactly one pitch-width left of the leftmost real card's position, a position the detector's candidate grid never samples — never a candidate, not filtered after the fact.
- Dead/eliminated players show: greyed-out name, "ELIMINATED" text, X overlaid on portrait, and no health-bar-colored name band — still present on the strip at their normal position (elimination doesn't remove or reflow a card), just excluded from `_player_alive`.

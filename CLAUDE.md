...
**`try_queue_favorite_reward(player_index)`** (on `MatchRunner`, called cross-thread from the Twitch bot's event loop — plain attribute mutation, no lock, same convention as this class's other cross-thread flags) accepts only if **all** of: nothing is already queued (only one redemption is ever allowed pending at a time, by explicit design choice — a second copy existing in the deck doesn't matter if one is already queued, a viewer must wait for the first to resolve), `player_index` names a real, currently-alive player in the match-start roster, and at least one `favorite_player` card remains unplayed in the deck. No cooldown, unlike POV — deck scarcity (typically 2 copies in the starting deck) is the natural rate limiter here.

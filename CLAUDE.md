...
**Current fix: trust requires a confirming second read, not a bounded jump.** A read that disagrees with the confirmed value — higher *or* lower, no asymmetry — is never trusted on the strength of one frame alone. `_update_points_reading()` immediately takes a second, independently-captured read (`self._read_points(take_screenshot())`) right then; only if it lands on that *exact* same value is the change accepted. A single frame disagreeing is exactly what a misread looks like (the `2 → 7` incident above would almost certainly not reproduce on an immediate second look); two independent frames landing on the same "surprising" number is real signal. This needs no assumption about the true regen rate at all — unlike a flat jump cap, it doesn't care how big the jump is or how long it's been. It's symmetric by construction too, so it subsumes the stuck-timeout's whole job: a wrong baseline (too high *or* too low) self-corrects the moment two consecutive real reads agree, with no separate escape-hatch mechanism needed — `_POINTS_MAX_JUMP_PER_READ`, `_POINTS_STUCK_TIMEOUT_SECONDS`, and `_last_points_update_time` are gone entirely. Costs an extra screenshot+OCR read, but only on an actual disagreement (the common "matches what we already believe" case still short-circuits with zero extra work). `tests/test_match_runner_points_ratchet.py` covers the agreement/disagreement shape in both directions, including a second read that disagrees (rejected) and one that agrees exactly (accepted) — a near-miss second read is deliberately *not* treated as agreement, exact match only.

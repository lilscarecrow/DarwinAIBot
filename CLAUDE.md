...
5. **All scopes this bot ever requests are combined into one constant**, `_OAUTH_SCOPES` in `bot/twitch_bot.py` (`_OAUTH_URL` is the full authorize URL built from it) — currently `user:bot`, `user:read:chat`, `user:write:chat`, `channel:bot` (chat), `channel:edit:commercial` (ads), `channel:read:subscriptions` (sub/resub/gift-sub shoutouts), `bits:read` (cheer shoutouts), `channel:read:redemptions` + `channel:manage:redemptions` (the channel-points POV reward), `moderation:read` (checking whether a channel-points redeemer is a mod, for the cooldown-bypass logic above), `channel:manage:predictions` (the "who wins" match prediction, see below). Visiting `_OAUTH_URL` (`http://localhost:4343/oauth?scopes=...`, logged automatically whenever any subscription attempt fails from a missing scope) grants all of them in one authorization — if the bot account and broadcaster are the same Twitch account (the current setup), one grant covers everything. Adding a new scope-gated feature later: add the scope to `_OAUTH_SCOPES` and re-authorize once; every existing grant stays valid, nothing needs to be redone.
...
**Live region-breakdown message below the signup message, not a `/role add` reply (2026-09-04, moved to a standing message 2026-09-09, numbered signup-order list 2026-09-11):** originally `_build_region_table()` appended a monospace table (Player / NA / EU columns, `X`/`-` per cell) to `/role add`'s response, covering only whoever that invocation had just assigned. It's now a standing message of its own — `_ensure_region_message()` posts it directly below the signup message (same channel, `scrim_signup_channel_id`) the first time both exist, and it tracks **every current signup reactor**, not just role-add's picks, so it reflects reality even before `/role add` has ever run. `_region_list_lines(guild, members)` (renamed from `_region_table_lines`) now builds a numbered list instead of a table — `"{i}. {name} — {tags}"` per member (`tags` is `NA`, `EU`, both, or omitted with no `—` suffix if neither role is held) — and returns `None` if neither region role exists in the guild, same "nothing to show" case as before. `_region_breakdown_embed()` wraps those lines in a small embed (`_COLOR_NEUTRAL`, plain text description, no code fence — a numbered list doesn't need monospace alignment the old column table did); `/role add`'s response still doesn't include any region info of its own.
...
`_give_first_blood_reward(killer_index, deck_pos)` is a thin wrapper around the shared `_give_reward_card()` (extracted 2026-09-10 when the Crowd Favorite reward below was added, since both need the exact same mechanics): locks POV the same way the match-start sequence does (see `SessionState._pov_locked`'s docstring — this reuses that same mechanism, not a separate lock), presses the target's `/pov` digit (`game/player_cards_v2.py::slot_number_for_index()`), then drops the card at **screen-center `(960, 540)`** — deliberately not through `_PLAYER_TARGETED_CARDS`'s `player_target_coordinates` system (never calibrated on any machine, see Player-Targeted Cards below): Darwin Project applies a "give X to player" card to whoever is currently spectated when dropped at center, the same point `electromania`/`beach_party`/etc. already use, so switching POV to the target first is what actually targets the reward at them. Builds a synthetic `CardEvent` to reuse `_play_tray_card()` (the exact same drag/verify/retry/announce logic every scheduled card goes through) rather than duplicating it. POV unlocks in a `finally` block regardless of whether the card play succeeds, so normal viewer POV control always resumes afterward.

...
**`placements` is a field DarwinStalker needs to add** — as of 2026-09-09 the results-screenshot response is documented as `{draft_id, game_index, ocr_error}` only (docs/DS_LIFECYCLE_HANDOFF.md). The bot-side code above assumes a `placements: [{rank, name, player_id}, ...]` field (finish order, present when `ocr_error` is `None`) will be added there — until it is, `_resolve_prediction()` always sees `placements=None` and every prediction gets canceled/refunded rather than resolved, which is the same safe fallback as any other unmatched case, not a separate failure mode.
